...
namespace Burrito
{
    public class EncounteredObject
    {
        ...
        public Vector2 position;   //The postion that the Obstacle will be drawn (x,y)
        public Vector2 size;       //The Size of the Obstacle (width,height)
        ...
        public EncounteredObject(Texture2D newTexture, Vector2 newPos)
        {
            ...
            hitbox = new Rectangle((int)(position.X + (size.X * .1)), (int)(position.Y + (size.Y * .1)), (int)(size.X * .9), (int)(size.Y * .9));
        }
        ...
        public bool WasHit(int x, int y, int width, int height)
        {
            ...
            //check if player.1 is between 1 & 3
